...
- Dependency lock refresh: [`SPECs/Agent/worksheet-dependency-lock-refresh.md`](SPECs/Agent/worksheet-dependency-lock-refresh.md) — refresh the compatible Rust dependency lockfile; defer the available `vite-plus` JavaScript toolchain lock update because it fails `vp check` with broad existing-test NodeNext/typechecking errors.
